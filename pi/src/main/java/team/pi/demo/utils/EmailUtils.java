...
public class EmailUtils {
    public static void sendEmail(User user){
        //发送方
        String myAccount = "cyt872592971@163.com";
        //授权码
        String myPass = "JITHBCCSCEUSZGXU";
        //发件人 邮箱的 SMTP 服务器地址
    ...
    private static MimeMessage createMsg(Session session, String myAccount, User user) {
        ...
        try {
            //3.1发件人 from
            message.setFrom(new InternetAddress(myAccount, "电影推荐系统", "utf-8"));
             ...
             * */
            message.setRecipient(MimeMessage.RecipientType.TO, new InternetAddress(user.getEmail(), user.getNickName(), "utf-8"));
            //3.3生成邮件主题
            message.setSubject("电影推荐系统激活邮件","utf-8");
            String ip = Inet4Address.getLocalHost().getHostAddress();
            String url = "http://127.0.0.1:8080/users/active?username="+user.getUserName()+"&c="+Base64Utils.encode(user.getCode());
            //设置邮件正文 setContent 可以使用html标签
            message.setContent(user.getUserName()+",您好<br>欢迎注册电影推荐系统! 请点击链接进行激活:<a href='"+url+"'>点击此处</a>","text/html;charset=utf-8");
            //设置邮件的发送时间 是立即发送
    ...
    //发送邮箱激活码
    public static void createCode(String userName ,String mail,int encode) {
        ...
        try {
            //3.1发件人 from
            message.setFrom(new InternetAddress(myAccount, "电影推荐系统验证码", "utf-8"));
            //3.2收件人 to 支持可以添加多个收件人 | 抄送 | 密送 如果想要发送给多个人 可以重复下面代码多次
            /*
             * MimeMessage.RecipientType.TO 发送
             * MimeMessage.RecipientType.CC 抄送
             * MimeMessage.RecipientType.BCC 密送
             * */
            message.setRecipient(MimeMessage.RecipientType.TO, new InternetAddress(mail, userName, "utf-8"));
            //3.3生成邮件主题
            message.setSubject("电影推荐系统账户验证码邮件","utf-8");
            //设置邮件正文 setContent 可以使用html标签
            message.setContent(userName+",您好<br>您的账户的验证码为: "+encode,"text/html;charset=utf-8");
            //设置邮件的发送时间 是立即发送
            message.setSentDate(new Date());
            //保存设置
            message.saveChanges();
